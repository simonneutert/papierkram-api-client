...
class IncomeProposition extends ApiV1Client {
  constructor(client) {
    super(client);
  }

  async by({ id }) {
    const url = this.buildBaseUri(`/income/propositions/${id}`);
  ...
  async all(config = {}) {
    ...
    const url = this.buildBaseUri(`/income/propositions`, params);
    const response = await fetch(url, {
      headers: {
        method: "GET",
        Accept: "application/json",
...
export default IncomeProposition;
